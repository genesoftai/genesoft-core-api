...
export class CloudWatchLogger extends ConsoleLogger {
 ...
 private isReady = false;
  ...
  private async setup() {
    ...
    const streams = await this.client.send(new DescribeLogStreamsCommand({
      logGroupName: this.logGroupName,
  ...
  async log(message: any, ...optionalParams: any[]) {
    super.log(message);
    await this.sendToCloudWatch(`[LOG] ${message} ${ JSON.stringify(optionalParams) ?? ''}`);
  }

  async error(message:any, ...optionalParams: any[]) {
    super.error(message);
    await this.sendToCloudWatch(`[ERROR] ${message} ${ JSON.stringify(optionalParams) ?? ''}`);
  }
